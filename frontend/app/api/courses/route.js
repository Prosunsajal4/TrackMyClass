...
export async function GET(request) {
  try {
    ...
    const courses = await Course.find({ user: decoded.id }).sort({ createdAt: -1 });
...
export async function POST(request) {
  try {
    ...
    const { courseName, totalClasses = 30 } = body;
    
    if (!courseName) {
      return NextResponse.json({ message: 'Please provide a course name' }, { status: 400 });
    }
