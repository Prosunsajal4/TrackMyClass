...
export default function CourseDetailPage() {
  ...
  const handleAttendanceToggle = async (section, classNumber, currentStatus) => {
    try {
      ...
      setCourse((prev) => {
        ...
        const updatedAttendance = prev[sectionKey].attendance.map((item) =>
          item.classNumber === classNumber
            ? { ...item, attended: !currentStatus, date: Date.now() }
            : item
        );
        
        const newAttendedCount = updatedAttendance.filter((item) => item.attended).length;
        ...
        return {
          ...
          stats: {
            ...prev.stats,
            [sectionKey === 'sectionA' ? 'sectionAPercentage' : 'sectionBPercentage']:
              (newAttendedCount / prev[sectionKey].total) * 100,
          },
        };
      });
  ...
  const exportToPDF = () => {
    ...
    doc.text(`Section A: ${course.stats.sectionAPercentage}% (${course.stats.sectionAMarks}/5)`, 20, 55);
    doc.text(`Section B: ${course.stats.sectionBPercentage}% (${course.stats.sectionBMarks}/5)`, 20, 65);
    
    doc.text('Section A Attendance:', 20, 80);
    let yPos = 90;
    course.sectionA.attendance.forEach((item) => {
      doc.text(`Class ${item.classNumber}: ${item.attended ? 'Present' : 'Absent'}`, 25, yPos);
      yPos += 7;
    });
  ...
  const currentSection = activeSection === 'A' ? course.sectionA : course.sectionB;
  const currentPercentage = activeSection === 'A' ? course.stats.sectionAPercentage : course.stats.sectionBPercentage;
  const currentMarks = activeSection === 'A' ? course.stats.sectionAMarks : course.stats.sectionBMarks;
        ...
        <main className="flex-1 ml-64 p-6">
          ...
          <div className="mt-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="glass-card rounded-xl p-4">
                <p className="text-sm text-gray-600 dark:text-gray-400">Total Attended</p>
            ...
            <div className="glass-card rounded-xl p-6">
              ...
              <div className="flex justify-between mt-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                <span>{currentSection.total - currentSection.attended} missed</span>
              </div>
            </div>
              ...
              <div className="glass-card rounded-xl p-6">
                ...
                <div className="space-y-3">
                  {course.suggestions.map((suggestion, index) => (
                    ...
                    >
                      <p className="text-gray-800 dark:text-white">{suggestion.message}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-5 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-3">
                {currentSection.attendance.map((item) => (
                  <button
                    ...
                    onClick={() => handleAttendanceToggle(activeSection, item.classNumber, item.attended)}
                  ...
                  >
                    {item.attended ? <Check className="w-6 h-6" /> : <X className="w-6 h-6" />}
